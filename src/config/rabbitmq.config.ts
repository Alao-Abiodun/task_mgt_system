...
interface IRabbitMQService {
    ...
    sendMessage(queueName: string, message: string, options?: any): Promise<void>;
    consumeMessages(queueName: string, onMessageCallback: (message: string, done: () => void) => void): Promise<void>;
...
export default class RabbitMQService implements IRabbitMQService {
  ...
  async connect() {
    try {
      this.connection = await amqp.connect(String(process.env.RABBITMQ_URL));
